...
| `icon-fast-forward` | 빨리감기 | `FastForward` | |
| `icon-favorite` | 좋아요/하트 | `Heart` | Material: `favorite` |
...
| `icon-sentiment-sad` | 감정(슬픔) | `Frown` | Material: `sentiment_sad` |
...
1. **lucide는 line 스타일, Material Symbols는 filled 지원** — 시각적 차이 있음
   - 특히 `icon-stop`, `icon-favorite`, `icon-check-circle`은 우리 프로토는 filled인 반면 lucide는 기본 outline
