...
| `icon-bolt` | 번개/액션 | `Zap` | Material: `bolt` |
| `icon-cancel-filled` | 취소/삭제 (filled) | `XCircle` + `fill` | Material: `cancel` (원형 X, filled) |
| `icon-castle` | 성/판타지 | `Castle` | Material: `castle` |
...
| `icon-close` | 닫기/X | `X` | Material: `close` |
| `icon-close-thin` | 닫기/X (thin) | `X` (strokeWidth 1.5) | Material: `close` thin variant |
| `icon-delete` | 삭제 (휴지통) | `Trash2` | Material: `delete` |
| `icon-drag-handle` | 드래그 핸들 (가로 2줄) | `GripHorizontal` | Material: `drag_handle` |
| `icon-drag-pan` | 드래그 팬 (4방향 화살표) | `Move` | Material: `drag_pan` |
...
| `icon-filter-vintage` | 꽃/장식 | `Flower` | Material: `filter_vintage` |
...
| `icon-help` | 도움말 | `HelpCircle` | Material: `help` |
...
| `icon-water-drop-filled` | 물방울 (filled) | `Droplet` + `fill="currentColor"` | Material: `water_drop` (filled) — viewer-end 「水をあげて応援する」 버튼 아이콘 |
...
| `icon-light-mode` | 라이트 모드/태양 | `Sun` | Material: `light_mode` |
...
| `icon-skull` | 해골/호러 | `Skull` | Material: `skull` |
...
| `icon-tool-tip` | 툴팁/도움말 | `HelpCircle` 또는 `Info` | 커스텀 — 안내 말풍선 |
| `icon-unfold-more` | 더 보기/확장 | `ChevronsUpDown` | Material: `unfold_more` |
| `icon-visibility` | 표시(눈) | `Eye` | Material: `visibility` |
| `icon-visibility-off` | 숨김(눈 꺼짐) | `EyeOff` | Material: `visibility_off` |
| `icon-visibility-on` | 표시(눈 켜짐) | `Eye` | Material: `visibility` (on variant) |
