...
### 15. Figma export 는 섹션 단위 정밀도 = 페이지 단위 정밀도
- 사용자가 **body 전체** Figma export 를 한 번에 주든, **카드 하나씩** 잘라 주든 — 추출하는 디테일의 정밀도는 **동일**해야 함
- 페이지 단위 export 라고 "기존 컴포넌트로 거의 비슷하게 매핑"하고 끝내지 말 것 — 각 섹션마다 다음을 항상 비교/추출:
  - 패딩(top·right·bottom·left 분리)
  - gap (각 flex 단계별)
  - 사이즈(고정 width/height — 포스터·아바타·아이콘 등)
  - radius / shadow
  - 버튼 사이즈/variant (min-height·padding·font 매칭으로 base/sm/xs 결정)
  - 폰트 토큰 (font-size + line-height + weight 조합으로 정확한 text-* 토큰 매핑)
- 페이지 export 처리 절차: ① 외곽 컨테이너 추출 → ② 각 섹션을 **개별 카드 단위로 분리해서 다시 본 듯이** 디테일 비교 → ③ 기존 컴포넌트와 어긋나는 항목은 페이지 전용 오버라이드로 정확히 반영
- 이유: "거의 같으니 컴포넌트 그대로" 로 처리하면 사용자가 다시 디테일을 짚어줘야 하는 두 번 작업 발생
